...
def should_run_now():
    # Allow manual/workflow runs anytime if set
...
def send_discord(msg):
    if not WEBHOOK:
        print("No webhook configured. Message preview:\n", msg)
        return
...
def normalize(raw, tickers):
    ...
    if isinstance(raw, pd.DataFrame) and not isinstance(raw.columns, pd.MultiIndex):
        ...
        if all(c in raw.columns for c in need): out[tkr]=raw[need].dropna().copy()
        return out
    if isinstance(raw, pd.DataFrame) and isinstance(raw.columns, pd.MultiIndex):
        for t in sorted({lv0 for lv0,_ in raw.columns}):
            df = raw[t].dropna().copy()
            need=["Open","High","Low","Close","Volume"]
            if all(c in df.columns for c in need): out[t]=df[need]
    return out
...
def is_etf(tkr):
    if tkr.upper() in ETF_TICKERS: return True
    try:
...
def daily_liquidity_ok(tkr):
    try:
        ...
        last=d.iloc[-1]
        px=_to_float(last["Close"]); avgv=_to_float(d["Volume"].tail(20).mean())
...
def levels_from_atr(price, atrp, bias):
    er = price*(atrp/100)*0.35; tg = price*(atrp/100)*1.10; st = price*(atrp/100)*0.70
    if bias=="CALL": return (round(price-er,2), round(price+er,2)), round(price+tg,2), round(price-st,2)
...
def main():
    ...
    for tkr, df in data.items():
        try:
            ...
            total=score_row(last,nsc); bias=bias_from_score(total)
            ...
            rows.append({"Ticker":tkr,"Price":round(price,2),"Score":round(float(total),2),"Type":bias,
                         ...
                         "Why":"; ".join(reasons),
                         "ok_contract":ok,"Option Contract":sym,"Strike":strike,"Opt Mid":mid,"Spread %":sp,
                         "Opt Vol":ov,"Opt OI":oi,"Opt Note":note})
    ...
    for _,r in view.iterrows():
        core=(f"- **{r['Ticker']}** @ ${r['Price']} → **{r['Type']}** | Exp: {r['Target Expiration']} "
              f"| Buy: {r['Buy Range']} | Target: {r['Sell Target']} | Stop: {r['Stop Idea']} "
              f"| Risk: {r['Risk']} | Why: {r['Why']}")
